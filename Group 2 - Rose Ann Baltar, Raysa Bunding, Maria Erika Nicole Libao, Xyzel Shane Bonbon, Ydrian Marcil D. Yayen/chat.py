...
with col1:
    send_message = st.text_area("Your message:", placeholder="Type your message here...", key="message_input")
    
with col2:
    st.write("")
    submit_btn = st.button("Send", use_container_width=True, type="primary")

if submit_btn and send_message:
    if not st.session_state.username:
        st.warning("Please enter your name in the sidebar before sending a message.")
    elif not send_message.strip():
        st.warning("Please enter a message before sending.")
    else:
        save_message(st.session_state.username, send_message.strip())
